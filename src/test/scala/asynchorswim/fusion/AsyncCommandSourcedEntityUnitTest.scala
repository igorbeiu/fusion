...
class AsyncCommandSourcedEntityUnitTest  extends AsyncFlatSpec with Matchers with BeforeAndAfterAll {
  ...
  private implicit val fc = FusionConfig(new FixedTimeProvider(now), asyncIO = true)
  ...
  "Async IO CommandSourcedEntity" should "handle normal messages" in {
    sut ! "inc"
    sut ! "inc"
    Thread.sleep(1000)
